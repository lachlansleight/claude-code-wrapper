...
The bridge currently emits two flat WebSocket frame types: `hook_event`
(raw, agent-tagged) and `state_event` (a derived personality state).
The new interface replaces these with a single `agent_event` frame
type. The personality state machine moves into consumers (the robot
firmware in particular). `hook_event` will remain for one release as a
compat shim; `state_event` goes away as soon as the firmware switches
over.

# Implementation Intentions

Sequenced plan for migrating the bridge from the current
`hook_event` + `state_event` shape to the spec above. Each step is
small enough to commit on its own.

## 1. Rip out personality from the bridge

- Delete `plugin/src/personality.ts` entirely.
- Remove `startPersonality()` call from `plugin/src/index.ts`.
- Remove the `state_event` listener + emit in `plugin/src/ws.ts` (and
  the `state_event` send-on-connect frame).
- Remove `state_event` from `plugin/src/bus.ts`'s `BusEvents`.
- Remove the `personality` block from `GET /api/state` in `http.ts`.
- Drop the `currentState()`/`timeInStateMs()` imports anywhere they
  appear.
- Personality stays only in `robot_v2/Personality.cpp` going forward.

## 2. Establish the new types module

- New file `plugin/src/agent-event.ts` exporting all the interfaces
  from the "AgentEvent — the classified event" section verbatim
  (`AgentEventEnvelope`, `AgentEvent` union, `ActivityKind`,
  `ActivityRef`, every `*Event` interface).
- New file `plugin/src/activity-classify.ts` holding a single
  `classifyTool(agent, toolName) → ActivityKind` function, driven by a
  per-agent lookup table mirroring the ActivityKind classification
  table at the bottom of this doc. Default branch returns
  `'tool.unknown'`.
- Move `toolAccess()` out of `plugin/src/adapters/types.ts` (it goes
  away — the consumer derives read/write from `ActivityKind` if it
  cares).

## 3. Rewrite the adapters as parsers

Each adapter becomes `parse(input) → AgentEventEnvelope[]` (note the
array — one native hook can fan out into multiple abstract events,
e.g. `Stop` → `message.assistant` + `turn.ended`, or `PostToolUse` for
`TodoWrite` → `activity.finished` + `todo.updated`).

Per-adapter notes for the rewrite:

- `claude.ts`
  - `UserPromptSubmit` → `[turn.started, message.user]`. Pull `prompt`
    from the payload.
  - `PreToolUse` → `activity.started`. Build `ActivityRef.summary`
    using a tiny per-tool detail formatter — port the relevant cases
    from `robot_v2/ToolFormat.cpp::detail()` (file path basename for
    Read/Edit/Write, command for Bash, query for WebSearch, etc.).
  - `PostToolUse` → `activity.finished`, plus `todo.updated` when
    `tool_name === 'TodoWrite'` (parse `tool_input.todos`).
  - `Stop` → `[message.assistant?, turn.ended]`. Keep the existing
    transcript-tail trick in `hook-forward.ts` for the assistant text;
    pass it through as `last_assistant_text`.
  - `SubagentStop` → `subagent.finished`.
  - `SessionStart` → `session.started` (cause from payload `source`).
  - `SessionEnd` → `session.ended`.
  - `PreCompact` → `context.compacting`.
  - `Notification` → `notification` always; additionally synthesise
    `permission.requested` when the message text contains a parseable
    request id (existing behaviour).

- `codex.ts`
  - Largely the same shape as `claude.ts` (Codex copied Claude's hook
    names). Differences:
    - `Stop.last_assistant_message` is provided directly — no
      transcript tail needed.
    - `PermissionRequest` → `permission.requested` (real id, not a
      synthesised one).
    - Tool name set: `Bash`, `apply_patch`, `Subagent`,
      `EditNotebook`, `GenerateImage`, `mcp__*`. Codex has no
      `TodoWrite` distinct from Claude's — it shares the name.
  - No `SessionEnd`, no `PreCompact`, no `SubagentStop`.

- `cursor.ts`
  - Subscribe canonical activity to `preToolUse` /
    `postToolUse` / `postToolUseFailure` only. **Do not** emit
    activities for `beforeShellExecution` / `afterShellExecution` /
    `beforeMCPExecution` / `afterMCPExecution` / `beforeReadFile` /
    `afterFileEdit` / `beforeTabFileRead` / `afterTabFileEdit` —
    those fire alongside `preToolUse`/`postToolUse` and would
    double-count.
  - Use the narrower events as a **summary enrichment side-channel**:
    cache the most recent `command`/`tool_input`/`file_path` keyed by
    `tool_use_id`, then read it when the matching `preToolUse` fires.
    Rough plan: small in-memory map `Map<tool_use_id, summaryHint>`
    populated from the narrow hooks, drained by the wide ones.
  - Special cases inside `preToolUse`:
    - `tool_name === 'ask_question'` → emit `agent.question` instead
      of `activity.started`. Pull text from `tool_input.question` (or
      whatever Cursor names it — confirm at parse time).
    - `tool_name === 'fetch_rules'` → drop entirely (not interesting).
  - `subagentStart` / `subagentStop` → `subagent.started` /
    `subagent.finished`.
  - `afterAgentResponse` → `message.assistant { final: true }`.
  - `afterAgentThought` → `thinking`.
  - `beforeSubmitPrompt` → `[turn.started, message.user]`.
  - `stop` → `turn.ended`.
  - `sessionStart` / `sessionEnd` → `session.started` / `session.ended`.
  - `preCompact` → `context.compacting`.

- `opencode.ts`
  - `session.created` / `session.deleted` → session events.
  - `session.idle` → `turn.ended`.
  - `session.compacted` → `context.compacting`.
  - `tool.execute.before` / `tool.execute.after` →
    `activity.started` / `activity.finished`. Tool-id table lives in
    `activity-classify.ts`; the classifier handles
    `read`/`write`/`edit`/`bash`/`grep`/`glob`/`webfetch`/`websearch`/`todo`.
  - `permission.asked` / `permission.replied` → permission events.
    Map response `'once' | 'always'` → `decision: 'allow'` with
    matching `scope`; anything else → `'deny'`.
  - `todo.updated` → `todo.updated`.
  - `message.updated` (role=user, first sighting) →
    `[turn.started, message.user]`.
  - `message.updated` (role=assistant, final part) →
    `message.assistant { final: true }`.
  - `message.part.updated` (type=thinking) → `thinking`.

## 4. Rewire HTTP + WS to the new envelope

- `http.ts::processAgentHook` becomes `processAgentHook` →
  iterates the parser output and `bus.emit('agent_event', envelope)`
  for each.
- `bus.ts` adds an `agent_event` channel. Drop `state_event`. Keep
  `hook_event` (raw, untransformed) for one release as a compat shim.
- `ws.ts` broadcasts every `agent_event` with `{ type:
  'agent_event', ... envelope }`. On new connect, no replay (events
  are inherently ephemeral; consumers reconnect-and-wait).
- Update the `examples/ws-client.html` to render `agent_event` frames
  with collapsible `raw` blocks for debugging.

## 5. Adjust state.ts bookkeeping

- `state.ts` keeps tracking active sessions (good for `/api/sessions`).
- Pending-permission map keyed by `request_id` survives but its
  payload becomes a slimmed-down `{ request_id, activity?,
  description? }` matching `PermissionRequestedEvent`.
- Drop `tool_name` / `description` / `input_preview` shape from
  `PendingPermission` in `types.ts`; replace with the spec shape.

## 6. Per-tool summary helpers

Port the relevant arms of `robot_v2/ToolFormat.cpp::detail()` into
TypeScript at `plugin/src/activity-summary.ts`:

- `Read|Edit|Write|MultiEdit` → `basename(file_path)`
- `Bash|run_terminal_cmd|Shell` → `command` (truncated 80 chars)
- `BashOutput`/`KillShell` → `bash_id`/`shell_id`
- `Glob|Grep|grep_search` → `pattern` or `query`
- `WebFetch|browser` → `host(url)`
- `WebSearch|web_search` → `query`
- `Task|Subagent` → `subagent_type` or task name
- `TodoWrite|todo` → `${count} items`
- `mcp__*` → tool name with `mcp__` stripped
- everything else → `''`

This is the source of `ActivityRef.summary`. Keep one shared file
across all four parsers — same logic, different vendor key names.

## 7. Documentation pass

- Replace this `# Implementation Intentions` section with a `# Status`
  section once the migration completes.
- Update `README.md`: drop the personality state list and the
  `state_event` row from the WS table; replace with `agent_event` and
  link to `OBJECT_INTERFACE.md`.
- Update `CLAUDE.md`: rewrite the "Bridge architecture" diagram to
  remove personality, note that personality lives in firmware.
- Update each `GETTING_STARTED_*.md` "Verify" section to look for
  `agent_event` frames, not `hook_event` + `state_event`.
- Drop any mention of `state_event` and `personality` from
  `OBJECT_INTERFACE.md` itself once the compat shim retires.

## 8. Firmware follow-up (not in this repo's bridge work)

- `robot_v2/Personality.cpp` keeps its state graph but starts
  consuming `agent_event` frames instead of raw hook events. Map:
  - `turn.started` → `THINKING` (via `routeToActive`)
  - `activity.started` with `kind` in `file.read|search.*|web.*` →
    `READING`
  - `activity.started` with `kind` in
    `file.write|file.delete|shell.*|notebook.edit|image.generate` →
    `WRITING`
  - `activity.finished` → arm tool linger then back to `THINKING`
  - `turn.ended` → `FINISHED`
  - `permission.requested`/`resolved` → `BLOCKED` overlay
  - `session.started` from `SLEEP` → `WAKING` → `READY`
- `ClaudeEvents` rename (it's not Claude-specific anymore): something
  like `AgentEvents` to match the new vocabulary.
- Out of scope for the bridge work; track separately.

## Open questions to resolve mid-implementation

- `agent.question.options[]`: Cursor's `ask_question` may or may not
  expose suggested answers in `tool_input` — confirm by reading a real
  payload before committing the parser.
- Cursor's exact `tool_use_id` correlation between narrow hooks
  (`beforeShellExecution`) and wide hooks (`preToolUse`) — verify
  they share the id before relying on it for summary enrichment.
- Whether `fetch_rules` should be dropped or surfaced as
  `tool.unknown`. Default to drop; revisit if a consumer wants it.
- Codex `apply_patch` payload shape — which fields tell us whether the
  patch is creating, editing, or deleting? Check before deciding
  whether `apply_patch` ever maps to `file.delete`.
