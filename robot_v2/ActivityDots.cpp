...
static void drawProgressDots(TFT_eSprite& s, uint16_t count, float baseRad,
                             float scale, float moodR, float moodG,
                             float moodB) {
  if (count == 0) return;
  if (count > kProgressMaxDots) count = kProgressMaxDots;
  if (scale <= 0.01f) return;
  const uint16_t dotColor =
      rgb888To565((uint8_t)moodR, (uint8_t)moodG, (uint8_t)moodB);
  ...
  for (uint16_t i = 0; i < count; ++i) {
    ...
    float argDeg =
        kProgressArcDegMin + (kProgressArcDegMax - kProgressArcDegMin) * countT;
    const float spanRad = argDeg * (float)PI / 180.0f;
    const float a = baseRad + (t - 0.5f) * spanRad;

    float currentRadius = kProgressDotRadiusMin +
                          (kProgressDotRadiusMax - kProgressDotRadiusMin) * countT;
...
void drawActivityDots(TFT_eSprite& s, const SceneRenderState& renderState,
  ...
  if (renderState.state == Personality::SLEEP) return;

  if (renderState.progress_fade_start_ms != 0) {
    const float t = (float)(now - renderState.progress_fade_start_ms) /
                    (float)kProgressFadeMs;
    const float scale = 1.0f - smoothstep01(t);
    drawProgressDots(s, renderState.fade_read_count, (float)PI / 2.0f, scale,
                     renderState.mood_r, renderState.mood_g, renderState.mood_b);
    drawProgressDots(s, renderState.fade_write_count, -(float)PI / 2.0f, scale,
                     renderState.mood_r, renderState.mood_g, renderState.mood_b);
    return;
  }

  if (renderState.state == Personality::IDLE) return;
  drawProgressDots(s, agentState.read_tools_this_turn, (float)PI / 2.0f, 1.0f,
                   renderState.mood_r, renderState.mood_g, renderState.mood_b);
  drawProgressDots(s, agentState.write_tools_this_turn, -(float)PI / 2.0f, 1.0f,
                   renderState.mood_r, renderState.mood_g, renderState.mood_b);
}
