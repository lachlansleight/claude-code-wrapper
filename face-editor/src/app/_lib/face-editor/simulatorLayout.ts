...
export const PARAM_LAYOUT: SliderSectionDef[] = PARAM_UI_SECTIONS.map(sec => ({
    section: sec.section,
...
> {
    const PARAM_RANGES: Record<string, { label: string; min: number; max: number; step: number }> =
        {};
...
export function buildModRanges(): Record<
    ...
    { label: string; min: number; max: number; step: number }
> {
    const MOD_RANGES: Record<string, { label: string; min: number; max: number; step: number }> =
        {};
