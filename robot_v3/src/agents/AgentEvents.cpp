...
static bool shellLikelyWrites(const char* summary) {
  ...
  if (containsToken(cmd, ">>") || containsToken(cmd, " >") || startsWithWord(cmd, ">") ||
      containsToken(cmd, "tee "))
    return true;

  const char* kWriteCmds[] = {
      "touch",     "mkdir",     "rmdir",     "rm",          "mv",
      "cp",        "install",   "truncate",  "chmod",       "chown",
      "ln",        "dd",        "sed -i",    "perl -i",     "npm install",
      "npm uninstall", "pnpm add", "pnpm remove", "yarn add", "yarn remove",
      "bun add",   "bun remove", "pip install", "pip uninstall",
  };
...
static void copyStreamBody(const char* src) {
  AsciiCopy::copyPreserveNewlines(g_state.body_text, sizeof(g_state.body_text), src ? src : "");
...
static void deriveTargets(const char* activity_kind, const char* summary) {
  ...
  if (activity_kind && containsText(activity_kind, "shell.exec")) {
    AsciiCopy::copy(g_state.latest_shell_command, sizeof(g_state.latest_shell_command), summary);
    return;
  }
  ...
  if (activity_kind && containsText(activity_kind, "file.write")) {
    AsciiCopy::copy(g_state.latest_write_target, sizeof(g_state.latest_write_target), summary);
    return;
  }
...
ActivityAccess classifyActivity(const char* activity_kind, const char* activity_tool,
...
static bool latchFilter(const char* session_id, const char* event_kind) {
  ...
  if (strcmp(g_state.latched_session, session_id) != 0) {
    if (event_kind &&
      ...
      LOG_EVT("session relatched via event: %s -> %s", g_state.latched_session, session_id);
      AsciiCopy::copy(g_state.latched_session, sizeof(g_state.latched_session), session_id);
      return true;
    }
    return false;
...
static void handleAgentEvent(JsonDocument& doc) {
  ...
  if (!strcmp(kind, "turn.started")) {
    ...
    if (!*decision) decision = evt["behavior"] | "";
    ...
    if (strcmp(g_state.status_line, "Done") != 0) {
      ...
      g_state.done_turn_elapsed_ms = (t0 != 0u && wall >= t0) ? (uint32_t)(wall - t0) : 0u;
    }
...
static void onActiveSessionsFrame(JsonDocument& doc) {
  ...
  for (size_t i = 0; i < ids.size(); ++i) {
    const char* s = ids[i] | "";
    if (*s && !strcmp(s, g_state.latched_session)) {
      found = true;
      break;
    }
  }
